...
public class ThreadLocal<T> {
     ...
     *
     * 修改当前线程与当前threadLocal对象相关联的 线程变量。
     *
     ...
     */
    // ThreadLocal就好像为每个线程提供了一个私人存物柜。每个线程都可以把自己的东西放进去，储存的变量或者其他的对象，对其他线程来说都是透明的。
    // ThreadLocal的自动清理是依靠Java中的弱引用（WeakReference）机制。
    //      在ThreadLocal中使用WeakReference作为key的方式，允许垃圾回收器在key不再被其他部分强引用时，回收这个key及其关联的值。
    //      当没有强引用指向某个对象时，该对象就成为垃圾的候选对象，随后在合适的时机被垃圾回收器回收。
    //      这个机制保证了ThreadLocal中存储的数据，当对应的线程结束生命周期后，会被及时地清理和回收，防止资源泄露或内存泄漏的问题。
    //      需要注意的是，虽然使用了WeakReference，但是没有使用引用队列（Reference Queue），因此过期的条目只有在表快要满时才会被自动清理。
